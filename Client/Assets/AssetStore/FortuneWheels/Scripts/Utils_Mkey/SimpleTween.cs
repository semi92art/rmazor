...
namespace Mkey
{
    public class SimpleTween : MonoBehaviour
    {
        ...
        {
            SetStartValue, SetEndValue, AsIs
        }
        
        private static List<SimpleTweenObject> _tweenObjects;
        private static SimpleTween _instance;
        private static float _pi = (float)Math.PI;
        private static float _pid2 = _pi / 2.0f;
        private int m_Length;
        private int m_FreeInd = -1;
        ...
        static void Create()
        {
            ...
            _instance = sT;
            _tweenObjects = new List<SimpleTweenObject>();
        ...
        {
            if (gO == null || _tweenObjects == null) return;
            List<SimpleTweenObject> sTOL = new List<SimpleTweenObject>();
            int length = _tweenObjects.Count;
        ...
        void Update()
        {
            m_Length = _tweenObjects.Count;
            if (m_Length == 0) return;
            m_FreeInd = -1;
            for (int i = 0; i < m_Length; i++)
            {
                _tweenObjects[i].Update(Time.deltaTime);
                if (_tweenObjects[i].IsFree) m_FreeInd=i;
            }

            if (m_FreeInd > -1 && m_Length > 50) _tweenObjects.RemoveAt(m_FreeInd);
        ...
        private static void Remove(int id)
        {
            ...
            for (int i = 0; i < length; i++)
            {
                if (_tweenObjects[i].ID == id) pos = i;
            }
            if (pos != -1) _tweenObjects.RemoveAt(pos);
        }

        private static SimpleTweenObject CreateTweenObject(GameObject gO, float start, float end, float time)
        {
            SimpleTweenObject sTO;
            int length = _tweenObjects.Count;// Debug.Log("tw length: " + length);
            for (int i = 0; i < length; i++)
            {
                if (_tweenObjects[i].IsFree)
                {
                    sTO = _tweenObjects[i];
        ...
        public class SimpleTweenObject
        {
            ...
            private float EaseInSine(float t, float c, float d)
            {
                if (Math.Abs(d) < float.Epsilon) 
                    return 1;
                return c * (1.0f - Mathf.Cos(t / d * _pid2));
            }
            ...
            private float EaseInOutSine(float t, float c, float d)
            {
                if (Math.Abs(d) < float.Epsilon) 
                    return 1;
                return c * (1.0f - Mathf.Cos(t / d * _pi)) * 0.5f;
            }
            #endregion sine
